...
//Version
const char version[15] { "Version 2.2" };
...
class AudioSynthFSKClock : public AudioStream {
public:
  AudioSynthFSKClock()
    : AudioStream(0, NULL) {
    currentPhaseStep = phaseStepForFrequency(kLowFrequencyHz);
  }

  void startClock(uint32_t symbolSamples) {
    if (symbolSamples == 0) {
      symbolSamples = 1;
    }

    pendingSymbolSamples = symbolSamples;
    pendingStart = true;
  }

  void updateSymbolSamples(uint32_t symbolSamples) {
    if (symbolSamples == 0) {
      symbolSamples = 1;
    }

    pendingSymbolSamples = symbolSamples;
    pendingTimingUpdate = true;
  }
  ...
  virtual void update(void) {
    ...
    if (pendingStop) {
      noInterrupts();
      pendingStop = false;
      pendingTimingUpdate = false;
      interrupts();

      outputEnabled = false;
      highSymbolActive = false;
      currentSymbolSamples = 1;
      nextSymbolSamples = 1;
      pendingSymbolUpdateAtBoundary = false;
      samplesUntilSymbolChange = 0;
      phase = 0.0f;
      currentPhaseStep = phaseStepForFrequency(kLowFrequencyHz);

      // Keep a queued start request if a new clock arrived before this audio
      // block ran, so rapid stop/start sequences can restart immediately.
    }

    if (pendingStart) {
      noInterrupts();
      const uint32_t localSymbolSamples = pendingSymbolSamples;
      pendingStart = false;
      pendingTimingUpdate = false;
      interrupts();

      outputEnabled = true;
      highSymbolActive = true;
      currentSymbolSamples = localSymbolSamples;
      nextSymbolSamples = localSymbolSamples;
      pendingSymbolUpdateAtBoundary = false;
      samplesUntilSymbolChange = localSymbolSamples;
      phase = 0.0f;
      currentPhaseStep = phaseStepForFrequency(kHighFrequencyHz);
    }

    if (pendingTimingUpdate) {
      noInterrupts();
      nextSymbolSamples = pendingSymbolSamples;
      pendingTimingUpdate = false;
      interrupts();
      pendingSymbolUpdateAtBoundary = true;
    }
    ...
    for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++) {
      ...
      block->data[i] = (int16_t)(sinf(phase) * kAmplitude * 32767.0f);

      if (samplesUntilSymbolChange > 0) {
        samplesUntilSymbolChange--;
        if (samplesUntilSymbolChange == 0) {
          highSymbolActive = !highSymbolActive;
          if (pendingSymbolUpdateAtBoundary) {
            currentSymbolSamples = nextSymbolSamples;
            pendingSymbolUpdateAtBoundary = false;
          }
          currentPhaseStep = phaseStepForFrequency(highSymbolActive ? kHighFrequencyHz : kLowFrequencyHz);
          samplesUntilSymbolChange = currentSymbolSamples;
        }
      }
    }
...
private:
  ...
  static constexpr float kLowFrequencyHz = 1300.0f;
  static constexpr float kHighFrequencyHz = 2100.0f;

  static float phaseStepForFrequency(float frequency) {
    return (2.0f * PI * frequency) / AUDIO_SAMPLE_RATE_EXACT;
  }

  volatile bool pendingStop = false;
  volatile bool pendingStart = false;
  volatile bool pendingTimingUpdate = false;
  volatile uint32_t pendingSymbolSamples = 1;

  bool outputEnabled = false;
  bool highSymbolActive = false;
  uint32_t currentSymbolSamples = 1;
  uint32_t nextSymbolSamples = 1;
  bool pendingSymbolUpdateAtBoundary = false;
  uint32_t samplesUntilSymbolChange = 0;
  float phase = 0.0f;
  float currentPhaseStep = 0.0f;
};
...
AudioConnection patchCord5(drum1, 0, mqsMixer, 0);
AudioConnection patchCord6(fskClock1, 0, mqsMixer, 1);
AudioConnection patchCord7(mqsMixer, 0, mqs1, 0);
AudioConnection patchCord8(mqsMixer, 0, mqs1, 1);
AudioConnection patchCord9(mixerLeft, 0, i2s2_1, 0);
AudioConnection patchCord10(mixerRight, 0, i2s2_1, 1);
...
int clock_resolution_value[9] = { 1, 6, 12, 24, 48, 96, 192, 384, 0 };
int baseOctaveMultiplierValue[5] = { 4, 16, 32, 64, 128 };
uint32_t lastFskClockMicros = 0;
uint32_t filteredFskClockIntervalMicros = 20833;
bool fskClockRunning = false;
bool fskClockHasMeasuredInterval = false;
uint32_t syncLastClockMicros[6] = { 0, 0, 0, 0, 0, 0 };
uint32_t syncFilteredClockIntervalMicros[6] = { 20833, 20833, 20833, 20833, 20833, 20833 };
uint32_t syncNextToggleMicros[6] = { 0, 0, 0, 0, 0, 0 };
uint32_t syncToggleIntervalMicros[6] = { 0, 0, 0, 0, 0, 0 };
uint8_t syncTogglesRemaining[6] = { 0, 0, 0, 0, 0, 0 };
bool syncClockHighState[6] = { false, false, false, false, false, false };
bool syncClockHasMeasuredInterval[6] = { false, false, false, false, false, false };
...
char clock_resolution_value_name[9][20] = {
  { "24ppqn" },
  { "sixteen" },
  { "eight" },
  { "quarter" },
...
void resetFskClockState() {
  lastFskClockMicros = 0;
  filteredFskClockIntervalMicros = 20833;
  fskClockRunning = false;
  fskClockHasMeasuredInterval = false;
  fskClock1.stopClock();
}

bool isFskClockSourceCable(byte cable) {
  return (Config.fskClockUsbPort > 0) && (cable == Config.fskClockUsbPort - 1);
}

bool isDoubledSyncResolution(int resolution) {
  return (resolution == 0) || (resolution == 8);
}

uint8_t syncSubdivisionsForResolution(int resolution) {
  if (resolution == 0) return 2;
  if (resolution == 8) return 4;
  return 0;
}

void resetSyncClockState(int port) {
  syncLastClockMicros[port] = 0;
  syncFilteredClockIntervalMicros[port] = 20833;
  syncNextToggleMicros[port] = 0;
  syncToggleIntervalMicros[port] = 0;
  syncTogglesRemaining[port] = 0;
  syncClockHighState[port] = false;
  syncClockHasMeasuredInterval[port] = false;
  digitalWriteFast(TRS_DATA_PIN[port], LOW);
}

void updateSyncClockDoublers() {
  const uint32_t now = micros();

  for (int i = 0; i < 6; i++) {
    if ((Config.TRS_MODE[i] != 2) || !isDoubledSyncResolution(Config.clock_resolution[i])) {
      continue;
    }

    while ((syncTogglesRemaining[i] > 0) && ((int32_t)(now - syncNextToggleMicros[i]) >= 0)) {
      syncClockHighState[i] = !syncClockHighState[i];
      digitalWriteFast(TRS_DATA_PIN[i], syncClockHighState[i] ? HIGH : LOW);
      syncTogglesRemaining[i]--;
      syncNextToggleMicros[i] += syncToggleIntervalMicros[i];
    }
  }
}
...
//current_parameter=Enc1, current_value=Enc2
  if (current_parameter != previous_parameter) {
    ...
    } else if (current_parameter == 19) {
      displayFskClockPort(Config.fskClockUsbPort);
...
//current_parameter=Enc1, current_value=Enc2
  if (current_value != previous_value) {
    ...
    */
    ...
    if ((current_value < 9) && (current_parameter >= 6) && (current_parameter < 12)) {
      Config.clock_resolution[current_parameter - 6] = current_value;
      displayClockResolution(current_parameter - 6, Config.clock_resolution[current_parameter - 6]);
    } else if ((current_value >= 9) && (current_parameter >= 6) && (current_parameter < 12)) {
      current_value = 8;
      Config.clock_resolution[current_parameter - 6] = current_value;
      displayClockResolution(current_parameter - 6, Config.clock_resolution[current_parameter - 6]);
    }
    ...
    //FSK USB Port Menu
    if ((current_value >= 0) && (current_value < 17) && (current_parameter == 19)) {
      Config.fskClockUsbPort = current_value;
      displayFskClockPort(Config.fskClockUsbPort);
      resetFskClockState();
    } else if ((current_value >= 17) && (current_parameter == 19)) {
      current_value = 16;
      Config.fskClockUsbPort = current_value;
      displayFskClockPort(Config.fskClockUsbPort);
      resetFskClockState();
...
void handleClock() {
  const byte cable = usbMIDI.getCable();

  if (isFskClockSourceCable(cable)) {
    const uint32_t currentMicros = micros();

    if (lastFskClockMicros != 0) {
      const uint32_t measuredIntervalMicros = currentMicros - lastFskClockMicros;
      if (measuredIntervalMicros >= 2000 && measuredIntervalMicros <= 250000) {
        if (!fskClockHasMeasuredInterval) {
          filteredFskClockIntervalMicros = measuredIntervalMicros;
          fskClockHasMeasuredInterval = true;
        } else {
          filteredFskClockIntervalMicros = (uint32_t)(((uint64_t)filteredFskClockIntervalMicros * 7 + measuredIntervalMicros + 4) / 8);
        }
      }
    }

    lastFskClockMicros = currentMicros;

    uint32_t symbolSamples = (uint32_t)((filteredFskClockIntervalMicros * AUDIO_SAMPLE_RATE_EXACT / 4000000.0f) + 0.5f);
    if (symbolSamples == 0) {
      symbolSamples = 1;
    }
    if (!fskClockRunning) {
      fskClock1.startClock(symbolSamples);
      fskClockRunning = true;
    } else {
      fskClock1.updateSymbolSamples(symbolSamples);
    }
  }

  // Only for managing TRS CLOCK SYNC output
  for (int i = 0; i < 6; i++) {
    if ((Config.TRS_MODE[i] == 2) && (cable == 8 + i)) {
      if (isDoubledSyncResolution(Config.clock_resolution[i])) {
        const uint32_t currentMicros = micros();

        if (syncLastClockMicros[i] != 0) {
          const uint32_t measuredIntervalMicros = currentMicros - syncLastClockMicros[i];
          if (measuredIntervalMicros >= 2000 && measuredIntervalMicros <= 250000) {
            if (!syncClockHasMeasuredInterval[i]) {
              syncFilteredClockIntervalMicros[i] = measuredIntervalMicros;
              syncClockHasMeasuredInterval[i] = true;
            } else {
              syncFilteredClockIntervalMicros[i] = (uint32_t)(((uint64_t)syncFilteredClockIntervalMicros[i] * 7 + measuredIntervalMicros + 4) / 8);
            }
          }
        }

        syncLastClockMicros[i] = currentMicros;

        uint8_t subdivisions = syncSubdivisionsForResolution(Config.clock_resolution[i]);
        uint32_t toggleIntervalMicros = syncFilteredClockIntervalMicros[i] / subdivisions;
        if (toggleIntervalMicros == 0) {
          toggleIntervalMicros = 1;
        }

        syncToggleIntervalMicros[i] = toggleIntervalMicros;
        syncNextToggleMicros[i] = currentMicros + toggleIntervalMicros;
        syncTogglesRemaining[i] = subdivisions - 1;
        syncClockHighState[i] = true;
...
void handleStop() {
  ...
  if (isFskClockSourceCable(usbMIDI.getCable())) {
    resetFskClockState();
  }
  for (int i = 0; i < 6; i++) {
...
void EpromLoad() {
  EEPROM.get(0, Config);
  for (int i = 0; i < 6; i++) {
    if (Config.clock_resolution[i] > 8) {
      Config.clock_resolution[i] = 0;
    }
  }
  if (Config.fskClockUsbPort > 16) {
    Config.fskClockUsbPort = 1;
  }
}
...
void displayFskClockPort(int value) {
  u8g2.clearBuffer();
  u8g2.setFontMode(1);
  u8g2.setBitmapMode(1);
  u8g2.setFont(u8g2_font_7x13_mf);
  u8g2.drawButtonUTF8(0, 15, U8G2_BTN_BW1 + U8G2_BTN_INV + U8G2_BTN_SHADOW2, 0, 2, 2, "FSK options");
  u8g2.setFontMode(0);

  u8g2.setFont(u8g2_font_6x13_tr);
  u8g2.drawStr(2, 47, "USB MIDI Port");

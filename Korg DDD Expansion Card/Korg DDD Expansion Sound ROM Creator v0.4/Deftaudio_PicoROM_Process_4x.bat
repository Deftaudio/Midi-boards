...
cls
echo ######################################################################
echo # Deftaudio PicoROM Korg DDD Expansion Sound ROM Encoder (4 samples) #
echo ######################################################################
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 "%folder%\%~1" ".\temp\%shortname%.wav" rate 44100 channels 1 silence 1 1 0.20% fade p 2s 8115s 50s
...
for /f "delims=" %%F in (temp\filelist.txt) do (
	type "tools\meta\4x_!count!.ddd" >> "%merged_file%"
...
type "tools\meta\4x_5.ddd" >> "%merged_file%"

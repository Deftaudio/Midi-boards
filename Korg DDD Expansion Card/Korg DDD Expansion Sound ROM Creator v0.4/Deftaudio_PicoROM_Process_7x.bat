...
cls
echo ######################################################################
echo # Deftaudio PicoROM Korg DDD Expansion Sound ROM Encoder (7 samples) #
echo ######################################################################
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 "%folder%\%~1" ".\temp\%shortname%.wav" rate 44100 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
...
for /f "delims=" %%F in (temp\filelist.txt) do (
	type "tools\meta\7x_!count!.ddd" >> "%merged_file%"
...
type "tools\meta\7x_8.ddd" >> "%merged_file%"
...
for /f "delims=" %%F in (temp\filelist.txt) do (
    ...
    type "temp\%%F" >> "%merged_file%"
		rem type "tools\meta\57.ddd" >> "%merged_file%"
)

REM Append the last 1 byte
type "tools\meta\1.ddd" >> "%merged_file%"

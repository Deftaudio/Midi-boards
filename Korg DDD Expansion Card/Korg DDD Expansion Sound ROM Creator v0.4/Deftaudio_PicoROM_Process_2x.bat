...
cls
echo ######################################################################
echo # Deftaudio PicoROM Korg DDD Expansion Sound ROM Encoder (2 samples) #
echo ######################################################################
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 "%folder%\%~1" ".\temp\%shortname%.wav" rate 44100 channels 1 silence 1 1 0.20% fade p 2s 16230s 50s
...
for /f "delims=" %%F in (temp\filelist.txt) do (
	type "tools\meta\2x_!count!.ddd" >> "%merged_file%"
...
type "tools\meta\2x_3.ddd" >> "%merged_file%"

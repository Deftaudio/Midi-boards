...
cls
echo ######################################################################
echo # Deftaudio PicoROM Korg DDD Expansion Sound ROM Encoder (6 samples) #
echo ######################################################################
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 "%folder%\%~1" ".\temp\%shortname%.wav" rate 44100 channels 1 silence 1 1 0.20% fade p 2s 5410s 50s
...
for /f "delims=" %%F in (temp\filelist.txt) do (
	type "tools\meta\6x_!count!.ddd" >> "%merged_file%"
...
type "tools\meta\6x_7.ddd" >> "%merged_file%"
...
REM Append last 159 bytes
REM type "tools\meta\159.ddd" >> "%merged_file%"

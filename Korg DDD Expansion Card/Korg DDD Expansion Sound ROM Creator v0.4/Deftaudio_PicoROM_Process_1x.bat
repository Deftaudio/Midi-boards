...
cls
echo #####################################################################
echo # Deftaudio PicoROM Korg DDD Expansion Sound ROM Encoder (1 sample) #
echo #####################################################################
echo.
...
if not exist .\tools\meta\1x_1.ddd (
	echo Critical Error: Metadata 1 cannot be found. Aborting!
	echo.
	pause
	EXIT /B
)
if not exist .\tools\meta\1x_2.ddd (
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 "%folder%\%~1" ".\temp\%shortname%.wav" rate 44100 channels 1 silence 1 1 0.20% fade p 2s 32460s 50s
...
for /f "delims=" %%F in (temp\filelist.txt) do (
	type "tools\meta\1x_!count!.ddd" >> "%merged_file%"
...
type "tools\meta\1x_2.ddd" >> "%merged_file%"

...
cls
echo ##############################################
echo # Korg DDD Expansion Sound ROM Encoder v0.30 # (8 samples)
echo ##############################################
echo.
...
if not exist .\[WAV_IN]\7.WAV (
	echo Critical Error: 7.WAV cannot be found. Aborting!
	echo.
	pause
	EXIT /B
)
if not exist .\[WAV_IN]\8.WAV (
	echo Critical Error: 8.WAV cannot be found. Aborting!
	echo.
	pause
	EXIT /B
)
echo Place eight WAV files into "[WAV_IN]" with the following naming scheme: 1.WAV, 
echo 2.WAV, 3.WAV, 4.WAV, 5.WAV, 6.WAV, 7.WAV, and 8.WAV. For ideal results, each 
echo sound should be shorter than 0.16 seconds in length. WAVs of any bit-depth or 
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\1.WAV .\temp\1_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\2.WAV .\temp\2_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\3.WAV .\temp\3_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\4.WAV .\temp\4_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\5.WAV .\temp\5_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\6.WAV .\temp\6_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\7.WAV .\temp\7_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\8.WAV .\temp\8_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4000s 50s
...
.\tools\soxdither\sox.exe -V1 .\temp\7_RT.WAV --bits 8 --encoding unsigned-integer .\temp\7.RAW dither -r
.\tools\soxdither\sox.exe -V1 .\temp\8_RT.WAV --bits 8 --encoding unsigned-integer .\temp\8.RAW dither -r

echo Merging singular raw waves into Korg DDD ROM image...
cd temp
copy /b 8x.ddd + 1.RAW + 57.ddd + 2.RAW + 57.ddd + 3.RAW + 57.ddd + 4.RAW + 57.ddd + 5.RAW + 57.ddd + 6.RAW + 57.ddd + 7.RAW + 57.ddd + 8.RAW + 61.ddd DDD_8X.BIN >NUL
move DDD_8X.BIN .. >NUL
cd..
echo.
...
if exist "DDD_8X.BIN" ( 
  for %%G in ("DDD_8X.BIN") do set "file_sizeA=%%~zG"
)

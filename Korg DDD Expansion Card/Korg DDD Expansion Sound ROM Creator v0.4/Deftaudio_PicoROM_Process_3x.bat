...
cls
echo ######################################################################
echo # Deftaudio PicoROM Korg DDD Expansion Sound ROM Encoder (3 samples) #
echo ######################################################################
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 "%folder%\%~1" ".\temp\%shortname%.wav" rate 44100 channels 1 silence 1 1 0.20% fade p 2s 10820s 50s
...
for /f "delims=" %%F in (temp\filelist.txt) do (
	type "tools\meta\3x_!count!.ddd" >> "%merged_file%"
...
type "tools\meta\3x_4.ddd" >> "%merged_file%"

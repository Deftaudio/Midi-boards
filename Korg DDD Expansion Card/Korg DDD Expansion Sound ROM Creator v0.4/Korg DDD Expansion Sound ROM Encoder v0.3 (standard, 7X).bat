...
cls
echo ##############################################
echo # Korg DDD Expansion Sound ROM Encoder v0.30 # (7 samples)
echo ##############################################
echo.
...
echo Place seven WAV files into "[WAV_IN]" with the following naming scheme: 1.WAV, 
echo 2.WAV, 3.WAV, 4.WAV, 5.WAV, 6.WAV, and 7.WAV. For ideal results, each sound 
echo should be shorter than 0.18 seconds in length. WAVs of any bit-depth or sample 
echo rate are allowed.
echo.
pause
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\1.WAV .\temp\1_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\2.WAV .\temp\2_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\3.WAV .\temp\3_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\4.WAV .\temp\4_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\5.WAV .\temp\5_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\6.WAV .\temp\6_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4637s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\7.WAV .\temp\7_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 4638s 50s
...
cd temp
copy /b 7x.ddd + 1.RAW + 2.RAW + 3.RAW + 4.RAW + 5.RAW + 6.RAW + 7.RAW DDD_7X.BIN >NUL
move DDD_7X.BIN .. >NUL
cd..
echo.
...
if exist "DDD_7X.BIN" ( 
  for %%G in ("DDD_7X.BIN") do set "file_sizeA=%%~zG"
)

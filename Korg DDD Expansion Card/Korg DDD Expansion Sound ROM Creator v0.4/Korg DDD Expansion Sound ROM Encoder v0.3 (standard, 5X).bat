...
cls
echo ##############################################
echo # Korg DDD Expansion Sound ROM Encoder v0.30 # (5 samples)
echo ##############################################
echo.
...
echo Place five WAV files into "[WAV_IN]" with the following naming scheme: 1.WAV, 
echo 2.WAV, 3.WAV, 4.WAV and 5.WAV. For ideal results, each sound should be shorter 
echo than 0.25 seconds in length. WAVs of any bit-depth or sample rate are allowed.
echo.
pause
echo.
...
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\1.WAV .\temp\1_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 6492s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\2.WAV .\temp\2_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 6492s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\3.WAV .\temp\3_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 6492s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\4.WAV .\temp\4_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 6492s 50s
.\tools\sox\sox.exe -V1 -D --norm=-0.3 .\[WAV_IN]\5.WAV .\temp\5_RT.WAV rate 25640 channels 1 silence 1 1 0.20% fade p 2s 6492s 50s
...
cd temp
copy /b 5x.ddd + 1.RAW + 2.RAW + 3.RAW + 4.RAW + 5.RAW DDD_5X.BIN >NUL
move DDD_5X.BIN .. >NUL
cd..
echo.
...
if exist "DDD_5X.BIN" ( 
  for %%G in ("DDD_5X.BIN") do set "file_sizeA=%%~zG"
)
